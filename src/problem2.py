...
class Triangle(object):
    ...
    def area(self):
        ...
        s1 = Point.distance_from(self.a, self.b)
        s2 = Point.distance_from(self.b, self.c)
        s3 = Point.distance_from(self.c, self.a)
        s = (1/2) * (s1 + s2 + s3)
        ss = s * (s - s1) * (s - s2) * (s - s3)
        area = math.sqrt(ss)
        return area
